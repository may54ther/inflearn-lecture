...
----
## 빈 스코프
빈이 존재 할 수 있는 범위로, 스프링은 빈 등록시 기본적으로 싱글톤 스코프로 생성

### 1. 싱글톤 스코프
- 컨테이너에게 빈을 요청할 때마다 **항상 동일한 객체**를 반환
- 스프링 컨테이너의 시작과 종료까지 유지되는 **가장 넓은 범위**의 스코프
> 싱글톤의 필드에는 의존관계에 있는 빈에 대한 레퍼런스 혹은 읽기 전용 값만 저장하고```DTO```와 같은 변수는 파라미터나 리턴값으로 전달하는 것이 바람직

### 2. 프로토타입 스코프
- 스프링 컨테이너에게 빈을 요청할 때마다 **항상 새로운 객체**를 생성하여 반환
- 스프링 컨테이너는 프로토타입 빈의 생성, 의존관계 주입, 초기화까지만 관여
  - 종료 메서드(```@PreDestroy```)가 호출되지 않음
  - 프로토타입 빈을 조회한 클라이언트가 직접 빈을 종료하거나 관리
...
> 📌싱글톤 내에서 **프로토타입 스코프** 혹은 **웹 스코프**를 함께 사용할 때 발생하는 문제 해결방법
> - ```ObjectProvider<T>```, ```Provider<T>``` 사용
> - Scope Proxy(프록시 패턴): ```@Scope(value = "스코프", proxyMode = ScopedProxyMode.TARGET_CLASS) ```
---
## 메모
